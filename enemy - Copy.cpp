...
Enemy::Enemy() : Soldier(), name("Enemy"), speed(80.f),
    ...
    fireTimer(0.f), fireCooldown(1.f) {
    x = 0;
    y = 0;
    width = 32;
    height = 48;
...
float Enemy::getPatrolRange() const {
    return patrolRange;
}
...
void Enemy::checkPlayerCollision(PlayerSoldier* player) const {
    (void)player;
}
...
void Enemy::checkEnemyCollision(Enemy* other) const {
    (void)other;
}
...
InfantryEnemy::InfantryEnemy() : Enemy(),
    ...
    fireCooldown(2.f), currentAnim(STAND) {
}
...
Paratrooper::Paratrooper() : AerialEnemy(), fireCooldown(5.5f), hasLanded(false) {
    name = "Paratrooper";
    hp = maxHp = 2;
    scoreValue = 75;
    flyHeight = -50.f;
...
float FlyingTara::setGroundY() const {
    return GroundY;
}
...
IronNokana::IronNokana() : missileLauncherActive(false),
    ...
    fireBombTimer(0.f), hasRetreated(false), fireCooldown(8.0f)
{
    name = "Iron Nokana";
    hp = maxHp = 30;
    scoreValue = 500;
