...
public:
        ...
        camera(screenX, screenY)
    {
        ...
        // Font load karo — dono paths try karo
...
private:
    ...
    void updateSurvival(float dt) {
        ...
        if (pX + characters.getActivePlayer()->getWidth() >= levelEndX - 10.f) {
            ...
            if (levelManager.getCurrentLevelIndex()
                < levelManager.getTotalLevels() - 1)
            {
                advanceToNextLevel();
                return;
            }
    ...
    void renderLevelTitle() {
        ...
        if (levelTitleTimer.getElapsedTime().asSeconds() < 3.0f) {
            ...
            Text t;
            t.setFont(font);
            t.setString(inBossLevel ? "BOSS LEVEL"
                : "LEVEL " + to_string(currentLevelNumber));
            t.setCharacterSize(100);
            t.setFillColor(inBossLevel ? Color::Red : Color::White);
            t.setStyle(Text::Bold);
            FloatRect tb = t.getLocalBounds();
            t.setPosition(screenX / 2.f - tb.width / 2.f, screenY / 2.f - 50.f);
            window.draw(t);
        }
        else {
