...
public:
    ...
    void crouch() {
        if (!inVehicle) { isCrouching = true; height = 50.0f; }
    }

    void freePrisoner() {}

    void applyFannumTax() {
        if (isFat) { width *= 1.5f; height *= 1.5f; }
    }
    ...
    void updateState() {
        if (stateTimer > 0) stateTimer -= 0.016f;
    }
    ...
    void update() {
        ...
        checkGrounded();
        updateState();
        if (meleeTimer > 0) meleeTimer -= 0.016f;
        if (specialPowerTimer > 0) {
            specialPowerTimer -= 0.016f;
            if (specialPowerTimer <= 0) specialPowerActive = false;
        }
    }
...
public:
    ...
    void updateDualFire() {
        if (dualFireTimer > 0) {
            ...
            if (dualFireTimer <= 0) dualFireActive = false;
        }
    }
...
public:
    ...
    void updateImmunity() {
        if (immunityTimer > 0) {
            ...
            if (immunityTimer <= 0) immunityActive = false;
        }
    }
...
public:
    ...
    void updateDoubleGrenade() {
        if (doubleGrenadeTimer > 0) {
            ...
            if (doubleGrenadeTimer <= 0) doubleGrenadeActive = false;
        }
    }
...
public:
    ...
    void updateSupercharged() {
        if (superchargedTimer > 0) {
            ...
            if (superchargedTimer <= 0) superchargedActive = false;
        }
    }
...
public:
    ...
    void update() {
        if (isActive && isSpawned) {
            spawnTimer -= 0.016f;
            if (isExpired()) { despawn(); return; }
            followPlayer();
            attackNearestEnemy();
        }
        if (cooldownTimer > 0) cooldownTimer -= 0.016f;
    }
...
private:
    ...
    void switchToIndexKeepingPosition(int index) {
        if (!isCharacterAvailable(index) || index == activeIndex) return;
...
public:
    ...
    void switchCharacter() {
        ...
        if (next != -1) switchToIndexKeepingPosition(next);
    }

    void switchCharacterToIndex(int index) {
        if (index < 0 || index >= 4 || !isCharacterAvailable(index)) return;
        PlayerSoldier* oldP = characters[activeIndex];
        PlayerSoldier* newP = characters[index];
    ...
    int getNextAvailableIndex() const {
        for (int i = 0; i < 4; i++) {
            ...
            if (isCharacterAvailable(next)) return next;
        }
        return -1;
    }
    ...
    bool allDead() const {
        for (int i = 0; i < 4; i++)
            if (characters[i] && characters[i]->getIsAlive()) return false;
        return true;
    }

    void resetAll() {
        for (int i = 0; i < 4; i++)
            if (characters[i]) characters[i]->respawn();
        activeIndex = 0;
    }

    void spawnFusion() {
        if (!fusionCompanion->canSpawn()) return;
        PlayerSoldier* alive[4]; int count = 0;
        for (int i = 0; i < 4; i++)
    ...
    void update() {
        if (characters[activeIndex]) characters[activeIndex]->update();
        if (fusionCompanion)         fusionCompanion->update();
    }

    void forceSetActiveIndex(int index) {
        if (index >= 0 && index < 4) activeIndex = index;
    }
