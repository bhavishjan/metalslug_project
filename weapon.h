...
public:
    ...
    float getX()     const { return x; }
    float getY()     const { return y; }
    bool  getIsExploded() const { return false; } // Rockets don't explode like grenades
...
public:
    ...
    void setIsExploded(bool exploded) { isExploded = exploded; }
};
...
private:
    ...
    int multiKillCount; // Track multi-kills for flawless victory

public:
    BulletManager() : multiKillCount(0) {}
    ...
    void checkMultiKill(float x, float y, ScoreSystem* scoreSystem) {
        // This method should only be called when enemies are actually killed
        // Multi-kills are handled in checkBulletEnemyCollisions when enemies die
        // No automatic scoring here to prevent false positives
    }
